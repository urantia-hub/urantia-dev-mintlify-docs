...
The API includes a built-in MCP server that exposes all 13 endpoints as tools — no setup, no API key, no local process:
...
Add this to Claude Desktop, Cursor, Claude Code, or any Streamable HTTP MCP client. Your agent gets 13 tools: search, semantic search, paragraphs, papers, entities, audio, and more.

<Card title="MCP Server Guide" icon="plug" href="/mcp">
  Full setup instructions, tool reference, and example prompts.
</Card>

### Docs MCP Server

Mintlify also provides a separate MCP server for searching these docs:
...
This gives your agent a `SearchUrantiaDev` tool for discovering endpoints and usage patterns. Use the **API MCP server** (`api.urantia.dev/mcp`) for actual data access, and the **docs MCP server** (`urantia.dev/mcp`) for learning how to use the API.
